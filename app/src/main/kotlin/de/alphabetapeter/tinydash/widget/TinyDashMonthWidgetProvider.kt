...
class TinyDashMonthWidgetProvider : AppWidgetProvider() {
	...
	companion object {
		...
		private val DEBUG_TAG: String = TinyDashMonthWidgetProvider::class.java.simpleName
		fun updateWidget(context: Context) {
			val widgetManager = AppWidgetManager.getInstance(context)
			val widgetComponent = ComponentName(context, TinyDashMonthWidgetProvider::class.java)
	...
	override fun onReceive(context: Context, intent: Intent) {
		...
		val thisAppWidget = ComponentName(context.packageName, TinyDashMonthWidgetProvider::class.java.name)
	...
	override fun onUpdate(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) {
		...
		for (widgetId in appWidgetIds) {
			...
			val svcIntent = Intent(context, TinyDashMonthRemoteService::class.java)

			svcIntent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, widgetId)
			svcIntent.data = Uri.parse(svcIntent.toUri(Intent.URI_INTENT_SCHEME))

			val remoteViews = RemoteViews(context.packageName, R.layout.widget_layout_month)
			remoteViews.setRemoteAdapter(R.id.calendarMonthEventList, svcIntent)

			appWidgetManager.notifyAppWidgetViewDataChanged(widgetId, R.id.calendarMonthEventList)
